...
## タイムライン（2/14午後〜2/17、2/17納品）

### 方針: パイプライン優先、待機時間を活用

P8パイプラインを最初に開始（自動処理の待機時間が最も長い）、待機時間にP7リファインメント、最後にP5/P6検証。

**優先順位:**
1. **タスク3（P8パイプライン）** → 最初に開始、自動ステップをバックグラウンド実行
2. **タスク2（P7 refine）** → パイプライン待機時間にカメラリファインメント
3. **タスク1（P5/P6検証）** → 最後、最も短時間で完了

**タスク4（アノテーション）** → **Kechuan**が別途担当

### 所要時間の見積もり（杉中さんのタスクのみ）
...
═══════════════════════════════════════════════════════════════════════
 2/14（金）午後のみ                                        Day 1
───────────────────────────────────────────────────────────────────────
 午後 │ タスク3 フェーズ0: P8 GoPro + mocapデータ整理       [1h]
      │ タスク3 フェーズ1: Mocap処理開始  ░░░░░░░░░░░       [自動 2h]
      │   ├── 待機中:
      │   │   タスク2: P7_4カメラrefine（cam1~cam6）        [3h]
───────────────────────────────────────────────────────────────────────
 完了: タスク3 フェーズ0+1自動完了、P7カメラ 6/14台refined
 作業時間: 約4h

═══════════════════════════════════════════════════════════════════════
 2/15（土）                                                Day 2
───────────────────────────────────────────────────────────────────────
 午前 │ タスク3 フェーズ1: ブレードアノテーション（手動）    [2.5h]
      │ タスク2: P7_4カメラrefine（cam7~cam12）             [3h]
      │
 午後 │ タスク3 フェーズ2: ブレード抽出  ░░░░░              [自動 1h]
      │   ├── 待機中:
      │   │   タスク2: P7_4カメラrefine（cam15~cam16）      [1h]
      │ タスク3 フェーズ2: cam19リファインメント             [1h]
───────────────────────────────────────────────────────────────────────
 完了: タスク3 フェーズ1+2完了、P7カメラ 14/14台refined ✓
 作業時間: 約7.5h
...
───────────────────────────────────────────────────────────────────────
 午前 │ タスク2: P7_1~P7_5にパラメータコピー + GT再分配     [1h]
      │ タスク2: P7セッションの同期検証 + カメラ修正        [2.5h]
      │ タスク3 フェーズ3: GoProパイプライン開始  ░░░░░░░░  [自動 3h]
      │   ├── 待機中（タスク2検証と並行）:
      │   │   タスク1: P5_3~P5_5の検証                      [1.5h]
      │
 午後 │ タスク3 フェーズ4: 全P8セッションの検証             [2h]
      │ タスク1: P6_1~P6_3の検証                            [1.5h]
───────────────────────────────────────────────────────────────────────
 完了: タスク2 完了 ✓、タスク3 完了 ✓、タスク1一部
 作業時間: 約7.5h

═══════════════════════════════════════════════════════════════════════
 2/17（月）                                                Day 4
───────────────────────────────────────────────────────────────────────
 午前 │ タスク1: P6_4~P6_5の検証                            [1h]
      │ タスク1: 問題カメラの修正（必要な場合）             [1~2h]
      │
 午後 │ バッファ: フラグ付きセッションの再確認              [約2h]
      │ 全participant横断のスポットチェック                  [約1h]
───────────────────────────────────────────────────────────────────────
 完了: タスク1 完了 ✓、全タスク完了 ✓
═══════════════════════════════════════════════════════════════════════
```
...
- **P8パイプライン最優先**: 自動処理（Mocap処理、ブレード抽出、GoProパイプライン）に計約6hの待機時間 → 早期開始で並行性を最大化
- **P7 refineで待機時間を活用**: 14台 × 30分をDay 1~2に分散、P8自動処理の待機時間に挟み込む
- **P5/P6検証は最後**: 最も短時間で完了するタスク（約5h）を重い作業の後に配置
- **2/17はバッファ多め**: コア作業はDay 3夕方までに完了目標、Day 4は修正と最終確認
...
| P1 ~ P8（全体） | 未着手 | **タスク4**: 動作時間アノテーション — **担当: Kechuan** |
...
## タスク4: データセット動作アノテーション（P1~P8） — 担当: Kechuan

> **注意**: このタスクはKechuanが担当します。杉中さんの作業ではありません。参考情報として記載。
