# CSL Pipeline 残作業計画

**担当: 杉中さん**
**作成日: 2026-02-09**
**期限: 2026-02-17**
**参考: `workflow/P7_complete_workflow_JP.md`（ツールの詳細な使い方）**

---

## タイムライン（2/10午後〜2/16作業、2/17納品）

### 方針: 検証完了ごとに即アノテーション

検証を全て終えてからアノテーションするのではなく、**各participantのデータが準備できた時点で即座にアノテーション**する。これにより段階的に成果物を産出できる。

- **P1~P4**: 処理済み → 最初にアノテーション（依存関係なし）
- **P5**: P5_3~P5_5を検証 → P5全セッションをアノテーション
- **P6**: P6_1~P6_5を検証 → P6全セッションをアノテーション
- **P7**: カメラrefine → P7全セッションをアノテーション
- **P8**: フルパイプライン → P8全セッションをアノテーション（最後）

### 所要時間の見積もり

| タスク | サブタスク | 推定時間 | 備考 |
|--------|----------|---------|------|
| **タスク1** | P5_3~P5_5 同期 + キャリブレーションチェック | 1.5h | 3セッション × 30分 |
| | P6_1~P6_5 同期 + キャリブレーションチェック | 2.5h | 5セッション × 30分 |
| | 問題カメラの修正（必要な場合） | 1~2h | YAMLコピーまたは再refine |
| | **タスク1 合計** | **約5h** | |
| **タスク2** | P7_4で14台のGoProカメラをrefine | 7h | 14台 × 30分（マーカーアノテーション + 最適化） |
| | パラメータコピー + GT再分配 | 1h | スクリプト実行、主に待機 |
| | 同期検証 + 個別カメラ修正 | 2h | 複数セッションをスポットチェック |
| | **タスク2 合計** | **約10h** | |
| **タスク3** | フェーズ0: P8データ整理 | 1h | 手動ファイル整理 |
| | フェーズ1: Mocap処理（自動） | 0.5hセットアップ + **2h待機** | パイプライン自動実行 |
| | フェーズ1: ブレードアノテーション（手動） | 2.5h | HTMLエディタ、セッションごと |
| | フェーズ2: ブレード抽出 + cam19 refine | 0.5h + **1h待機** + 1h refine | cam19マーカーアノテーション |
| | フェーズ3: GoProパイプライン（自動） | 0.5hセットアップ + **3h待機** | 同期 + キャリブレーション + GT |
| | フェーズ4: 全P8セッションの検証 | 2h | 同期 + キャリブレーションチェック |
| | **タスク3 合計** | **約8h手動 + 約6h待機** | 待機時間は他タスクに活用可能 |
| **タスク4** | アノテーション P1~P8（約36セッション） | 6h | 1セッションあたり約10分 |
| | **タスク4 合計** | **約6h** | |
| | **総合計** | **約29h手動 + 約6h待機** | |

### 日別スケジュール

```
═══════════════════════════════════════════════════════════════════════
 2/10（月）午後のみ                                        Day 1
───────────────────────────────────────────────────────────────────────
 午後 │ タスク3 フェーズ0: P8データ整理                      [1h]
      │ タスク3 フェーズ1: Mocap処理開始  ░░░░░░░░░░░░░     [自動 2h]
      │   ├── 待機中:
      │   │   ★ P1をアノテーション（データ準備済み）        [約1h]
      │   │   ★ P2をアノテーション（データ準備済み）        [約1h]
───────────────────────────────────────────────────────────────────────
 完了: タスク3 フェーズ0 + フェーズ1自動、P1✓ P2✓ アノテーション済み
 作業時間: 約3h
 アノテーション進捗: ██░░░░░░ P1✓ P2✓

═══════════════════════════════════════════════════════════════════════
 2/11（火）                                                Day 2
───────────────────────────────────────────────────────────────────────
 午前 │ ★ P3 + P4をアノテーション（データ準備済み）        [約2h]
      │ タスク1: P5_3~P5_5の検証                            [1.5h]
      │
 午後 │ ★ P5をアノテーション（検証直後）                   [約1h]
      │ タスク3 フェーズ1: ブレードアノテーション（手動）    [2.5h]
───────────────────────────────────────────────────────────────────────
 完了: P3✓ P4✓ P5✓ アノテーション済み、タスク1一部
 作業時間: 約7h
 アノテーション進捗: █████░░░ P1✓ P2✓ P3✓ P4✓ P5✓

═══════════════════════════════════════════════════════════════════════
 2/12（水）                                                Day 3
───────────────────────────────────────────────────────────────────────
 午前 │ タスク1: P6_1~P6_5の検証 + 修正                    [3.5h]
      │
 午後 │ ★ P6をアノテーション（検証直後）                   [約1h]
      │ タスク3 フェーズ2: ブレード抽出  ░░░░░  [自動 1h]
      │ タスク3 フェーズ2: cam19リファインメント             [1h]
      │ タスク3 フェーズ3: GoProパイプライン開始  ░░░  [自動、夜間実行]
───────────────────────────────────────────────────────────────────────
 完了: タスク1 完了 ✓、P6✓ アノテーション済み、タスク3 フェーズ2+3開始
 作業時間: 約5.5h
 アノテーション進捗: ██████░░ P1✓ P2✓ P3✓ P4✓ P5✓ P6✓

═══════════════════════════════════════════════════════════════════════
 2/13（木）                                                Day 4
───────────────────────────────────────────────────────────────────────
 午前 │ タスク2: P7_4カメラrefine（cam1~cam8）              [4h]
      │   （タスク3 フェーズ3自動はこの間に完了）
      │
 午後 │ タスク2: P7_4カメラrefine（cam9~cam18）             [3h]
───────────────────────────────────────────────────────────────────────
 完了: タスク2 refine完了、タスク3 フェーズ3自動完了
 作業時間: 約7h

═══════════════════════════════════════════════════════════════════════
 2/14（金）                                                Day 5
───────────────────────────────────────────────────────────────────────
 午前 │ タスク2: パラメータコピー + GT再分配 + 同期検証     [2.5h]
      │ タスク2: 個別問題カメラの修正                       [0.5h]
      │ ★ P7をアノテーション（refine直後）                 [約1h]
      │
 午後 │ タスク3 フェーズ4: 全P8セッションの検証             [2h]
      │ ★ P8をアノテーション（検証直後）                   [約1h]
───────────────────────────────────────────────────────────────────────
 完了: タスク2 完了 ✓、タスク3 完了 ✓、P7✓ P8✓ アノテーション済み
 作業時間: 約7h
 アノテーション進捗: ████████ P1✓ P2✓ P3✓ P4✓ P5✓ P6✓ P7✓ P8✓

═══════════════════════════════════════════════════════════════════════
 2/15〜16（土〜日）                                     Day 6〜7
───────────────────────────────────────────────────────────────────────
      │ CSV最終レビュー + 整合性チェック                    [0.5h]
      │ バッファ: アノテーション中に発見した問題の修正      [約2h]
───────────────────────────────────────────────────────────────────────
 完了: 全タスク完了 ✓
═══════════════════════════════════════════════════════════════════════
```

### 設計上の重要ポイント

- **P1~P4を最優先でアノテーション**: データ準備済みで依存関係なし。Day 1~2で即着手し、早期に成果を産出
- **検証→即アノテーション**: P5/P6/P7/P8は、検証・refine完了後にすぐアノテーション。別のバッチにまとめない
- **タスク2（P7 refine）を集中日に**: 14台 × 30分は最大の手動作業ブロック、Day 4に中断なしで実施
- **タスク3の自動待機時間**: 約6hのパイプライン待機時間をアノテーション（Day 1）と検証（Day 3）に活用
- **Day 6〜7はバッファのみ**: 最終レビュー + 修正のみ、コア作業はDay 5までに完了

---

## 概要

| 参加者 | 状態 | 残作業 |
|--------|------|--------|
| P4_1 ~ P5_2 | 完了 | なし |
| P5_3, P5_4, P5_5 | カメラパラメータ + GT生成済み | キャリブレーション＆同期の検証 |
| P6_1 ~ P6_5 | カメラパラメータ + GT生成済み | キャリブレーション＆同期の検証 |
| P7_1 ~ P7_5 | パイプライン完了 | 個別GoProパラメータのrefine + 同期チェック |
| P8_1 ~ P8_? | 未着手 | フルパイプライン処理 |
| P1 ~ P8（全体） | 未着手 | **タスク4**: 動作時間アノテーション（cam1 GTオーバーレイ → CSV） |

## データ配置

| ボリューム | 内容 | パスパターン |
|-----------|------|-------------|
| **T7** | P4-P6 Mocap出力（スケルトン、マーカー、ブレードエッジ、cam19_refined.yaml） | `/Volumes/T7/csl/PX_Y/` |
| **T7** | P7 Mocap出力（P7_output） | `/Volumes/T7/P7_output/P7_X/` |
| **HumanDATA** | 同期済みGoProビデオ、キャリブレーション、個別カメラパラメータ、GTフォルダ | `/Volumes/HumanDATA/Prohuman/synced/PX_Y_sync/cameras_synced/` |

---

## タスク1: P5_3, P5_4, P5_5 および P6_1~P6_5 の検証

これらのセッションにはカメラパラメータとGTデータが既に配置されています。以下を検証する必要があります：
1. **キャリブレーション（投影）**: スケルトン関節がビデオ内の人体と一致しているか
2. **同期（タイミング）**: 時間的な遅延や進みがないか

### 手順

各セッション（P5_3, P5_4, P5_5, P6_1, P6_2, P6_3, P6_4, P6_5）について：

#### 1. 同期チェック

```bash
python post_calibration/verify_gt_offset.py \
    --session_dir /Volumes/HumanDATA/Prohuman/synced/PX_Y_sync/cameras_synced \
    --camera cam1 \
    --start 60 \
    --duration 10
```

- ビデオを再生し、スケルトンオーバーレイが時間的に整列しているか確認
- タイミングがずれている場合：`[` / `]` でオフセット調整後、`e` で保存してGT自動再分配
- 複数のカメラ（cam1, cam5, cam10など）をチェック

#### 2. キャリブレーションチェック（投影品質）

同期検証ツール使用中に、スケルトンが人体と空間的にも一致しているか確認（タイミングだけでなく）。

**特定のカメラのキャリブレーションがおかしい場合：**

以下のトラブルシューティング手順に従ってください：

1. **前のセッションからパラメータをコピー**: 例えばP5_3/cam3がおかしい場合、P5_2の`individual_cam_params/`から`cam3.yaml`をP5_3にコピー
2. **まだおかしい場合、次のセッションを試す**: P5_4の`individual_cam_params/`から`cam3.yaml`をコピー
3. **まだおかしい場合、再refine**: インタラクティブリファインメントツールを実行：
   ```bash
   python post_calibration/refine_extrinsics.py \
       --session PX_Y --cam camN \
       --markers-base /Volumes/T7/csl \
       --synced-base /Volumes/HumanDATA/Prohuman/synced
   ```
4. カメラYAMLを更新した後、そのセッションのGTを再分配

### P5_4に関する注意

P5_4には**body_markers.npyがありません**。そのため`refine_extrinsics.py`を実行できません。P5_4でキャリブレーションに問題がある場合：
- **P5_3**または**P5_5**の対応するカメラYAMLを使用してください
- YAMLファイルをコピーし、投影を目視確認すれば完了

### カメラ修正のワークフロー

```
PX_Y / camN で問題を検出
    ↓
PX_(Y-1) から camN.yaml を PX_Y/individual_cam_params/ にコピー
    ↓
再可視化 → 投影OK？
    ├── はい → 完了
    └── いいえ → PX_(Y+1) から camN.yaml をコピー
                  ↓
                  再可視化 → OK？
                  ├── はい → 完了
                  └── いいえ → refine_extrinsics.py を実行（body_markers.npyがある場合）
                                ↓
                                GTを再分配
```

---

## タスク2: P7 個別GoProリファインメント

P7パイプラインは完了済み（同期ビデオ、キャリブレーション、GT分配）。ただし、個別GoProカメラパラメータは**未refine**で、共同キャリブレーションからの直接出力です。

### 手順

#### 1. P7_4で全GoProカメラをrefine

P7_4は動作範囲が広く、body_markers.npyがあります。各GoProカメラ（cam1~cam18、cam19はスキップ）をrefine：

```bash
# 各カメラについて（cam1, cam2, cam3, ..., cam18、cam13/cam14はスキップ）：
python post_calibration/refine_extrinsics.py \
    --session P7_4 --cam camN \
    --markers-base /Volumes/T7/P7_output \
    --synced-base /Volumes/HumanDATA/Prohuman/synced
```

各カメラについて：
1. `f` を押してマーカーが見えるクリアなフレームを検索
2. 3Dマーカーを左クリック、ビデオ内の対応位置を右クリック
3. 最低6-8個のマーカーペアをアノテーション
4. `O` を押して最適化（scipy、推奨）
5. `[`/`]` でフレーム間を確認
6. `e` を押してエクスポート

#### 2. refine済みパラメータを他のP7セッションにコピー

P7_4で全カメラをrefineした後、YAMLを他のセッションにコピー：

```bash
# P7_4のindividual_cam_paramsをP7_1, P7_2, P7_3, P7_5にコピー
for session in P7_1 P7_2 P7_3 P7_5; do
    cp /Volumes/HumanDATA/Prohuman/synced/P7_4_sync/cameras_synced/individual_cam_params/cam*.yaml \
       /Volumes/HumanDATA/Prohuman/synced/${session}_sync/cameras_synced/individual_cam_params/
done
```

#### 3. 全P7セッションのGTを再分配

カメラパラメータ更新後、GTの再分配が必要：

```bash
for session in P7_1 P7_2 P7_3 P7_4 P7_5; do
    python scripts/distribute_gt.py \
        --session_dir /Volumes/HumanDATA/Prohuman/synced/${session}_sync/cameras_synced
done
```

#### 4. 同期を検証

```bash
python post_calibration/verify_gt_offset.py \
    --session_dir /Volumes/HumanDATA/Prohuman/synced/P7_1_sync/cameras_synced \
    --camera cam1
```

複数のセッションの複数のカメラをチェック。タイミングがずれている場合は調整して`e`。

#### 5. 個別問題カメラの修正（必要に応じて）

P7_4のパラメータをコピーした後、特定のセッションの特定のカメラがおかしい場合：
- そのセッションでそのカメラを個別にrefine
- または隣接セッションのパラメータを試す

---

## タスク3: P8 フルパイプライン処理

P8は新しい参加者で、ゼロからの完全処理が必要です。Mocapデータはエクスポート済みです。

### 前提条件

1. **P8 mocapデータを確認**: KINGSTONまたはT7でエクスポート済みMotiveデータ（AVI + CSV + .mcal）を見つける
2. **P8 GoProデータを確認**: 生GoProビデオ（カメラごと構造）を見つける

### 手順

`workflow/P7_complete_workflow_JP.md`の完全ワークフローに従い、P7をP8に置き換えてください：

#### フェーズ0: データ準備

1. **mocapデータを標準構造に整理**:
   ```
   P8_mocap/
   ├── P8_1/
   │   ├── *.avi          (PrimeColorビデオセグメント)
   │   └── *.csv          (Motiveエクスポート)
   ├── P8_2/, P8_3/, ...
   └── *.mcal             (OptiTrackキャリブレーション、親ディレクトリ)
   ```

2. **GoProデータをカメラごとからセッションごとに整理**:
   ```bash
   python workflow/organize_gopro_videos.py \
       --input /path/to/P8_gopro \
       --output /Volumes/FastACIS/csl_11_5/organized \
       --participant P8 \
       --dry-run
   # 確認後、--dry-runなしで実行
   ```

#### フェーズ1: Mocap処理

```bash
python workflow/process_mocap_session.py \
    /path/to/P8_mocap \
    -o /path/to/P8_output \
    --batch
```

その後：ブレードHTMLエディタを開き、エッジをアノテーション、JSONをエクスポート（詳細はワークフローのドキュメント参照）。

#### フェーズ2: ブレード抽出 + cam19リファインメント

```bash
python workflow/process_blade_session.py \
    /path/to/P8_mocap \
    -o /path/to/P8_output \
    --batch \
    --share_json P8_1
```

その後：`refine_extrinsics.py`をcam19に対して実行（動作範囲が最も広いセッションを選択）。

#### フェーズ3: GoPro完全パイプライン

```bash
python workflow/process_p7_complete.py \
    --organized_dir /Volumes/FastACIS/csl_11_5/organized \
    --mocap_dir /path/to/P8_output \
    --output_dir /Volumes/FastACIS/csl_11_5/synced \
    --anchor_video /Volumes/FastACIS/csl_11_5/organized/qr_sync.mp4 \
    --calibration_session P8_2 \
    --start_time <charuco開始秒数> \
    --duration <charuco持続秒数> \
    --sessions P8_1 P8_2 P8_3 P8_4 P8_5
```

`--calibration_session`、`--start_time`、`--duration`の決め方：GoProビデオを開いてChArucoボードが静止している時間帯を見つけてください。ワークフローのドキュメントの「決め方」セクションを参照。

#### フェーズ4: 検証

上記タスク1・2と同様：全セッション・全カメラのキャリブレーション + 同期を検証。

---

## ヒントとコツ

### インタラクティブツール キーリファレンス

| キー | 機能 |
|------|------|
| `f` | 安定フレーム自動検索（refine_extrinsics） |
| `Space` | 再生/一時停止 |
| `[` / `]` | 前/次のフレーム |
| `,` / `.` | オフセット調整 または 10フレームジャンプ |
| `O` | scipy最適化（推奨） |
| `P` | solvePnP最適化 |
| `e` | エクスポート / 保存 |
| `u` | 最後のマーカーペアを取り消し |
| `c` | 全ペアクリア |
| `q` | 終了 |

### AIに質問するべき場合

- `refine_extrinsics.py`が収束しない場合（8+マーカーペアでもエラーが大きい）
- P8キャリブレーションの`--start_time`/`--duration`が分からない場合
- GT分配が失敗したり予期しない結果になった場合
- コマンドの構文について不明な点がある場合は `workflow/P7_complete_workflow_JP.md` を参照

---

## タスク4: データセット動作アノテーション（P1~P8）

全セッション（P1からP8まで）の各動作の時間区間をアノテーションし、構造化されたデータセットインデックスを作成します。最終成果物は1つのCSVファイルです。

### 出力フォーマット

単一CSVファイル: `dataset_annotations.csv`（プロジェクトルートまたは共有の場所に保存）

カラム:

| action | session | S1 | E1 | S2 | E2 | S3 | E3 | ... |
|--------|---------|-----|-----|-----|-----|-----|-----|-----|
| running on treadmill | P1_1 | 12.5 | 45.0 | 80.2 | 110.0 | | | |
| walking | P1_1 | 120.0 | 155.3 | | | | | |
| running on treadmill | P2_3 | 5.0 | 38.0 | | | | | |

- **action**: 事前定義された動作名（実験プロトコルの完全なリストを参照）
- **session**: 該当セッション（例: P2_3）
- **S1, E1, S2, E2, ...**: **cam1ビデオの秒数**（0:00からの絶対時間）での開始・終了時刻。同じ動作が複数回行われた場合や、品質の問題で区間が分断される場合は複数区間を記録
- **品質合格の区間のみ**を記録（品質基準は下記参照）

### 手順

各セッション（P1_1, P1_2, ..., P8_N）について：

#### 1. cam1ビデオをGTオーバーレイ付きで開く

```bash
python post_calibration/verify_gt_offset.py \
    --session_dir /Volumes/HumanDATA/Prohuman/synced/PX_Y_sync/cameras_synced \
    --camera cam1 \
    --start 0 \
    --duration -1
```

`Space`で再生/一時停止、`[`/`]`でフレーム送り。

#### 2. 動作区間の特定

ビデオを視聴し、各事前定義動作の開始・終了を特定する。各開始/終了ポイントの**cam1ビデオのタイムスタンプ**（プレイヤーに表示）を記録。

#### 3. データ品質チェック

各区間について、記録する前に以下を確認：

- **義足がフレーム内に収まっている**: ブレード/義足がcam1ビューで区間全体を通して見えている
- **GTスケルトンが正しく整列している**: 飛ぶ関節、急なジャンプ、(0,0,0)に固定された関節がない
- **被験者が期待される動作を実行している**: 動きが動作名の説明と一致する
- **大きな遮蔽がない**: 被験者の体が十分に見える

**品質に問題がある区間**: CSVに含めない。区間の一部のみ良好な場合、サブ区間に分割して良好な部分のみ記録（例: 25-30フレームが不良なら S1=10, E1=25, S2=30, E2=50）。

#### 4. CSVに記録

有効な動作区間ごとに、`dataset_annotations.csv`に以下を記入：
- 動作名
- セッションID（例: P3_2）
- 各有効サブ区間の開始/終了秒数

### 注意事項

- **cam1のみ**で全アノテーション作業が完結
- セッション数は参加者ごとに異なる（全員が5セッションとは限らない）
- 各参加者の完全な動作リストを確認するには、実験プロトコルまたはビデオの内容を確認
- 同じ動作が複数セッションに出現したり、1セッションに複数の異なる動作が含まれることは正常
- データ品質に疑問がある場合は、その区間を除外する方向で判断

---

## チェックリスト

### P5 + P6 検証
- [ ] P5_3: 同期チェック + キャリブレーションチェック
- [ ] P5_4: 同期チェック + キャリブレーションチェック（refine不可、必要ならP5_3/P5_5のパラメータ使用）
- [ ] P5_5: 同期チェック + キャリブレーションチェック
- [ ] P6_1: 同期チェック + キャリブレーションチェック
- [ ] P6_2: 同期チェック + キャリブレーションチェック
- [ ] P6_3: 同期チェック + キャリブレーションチェック
- [ ] P6_4: 同期チェック + キャリブレーションチェック
- [ ] P6_5: 同期チェック + キャリブレーションチェック

### P7 リファインメント
- [ ] P7_4で全GoProカメラをrefine（cam1~cam18、cam13/14/19はスキップ）
- [ ] refine済みパラメータをP7_1, P7_2, P7_3, P7_5にコピー
- [ ] 全P7セッションのGTを再分配
- [ ] P7セッションの同期を検証
- [ ] 個別問題カメラを修正

### P8 フル処理
- [ ] P8 mocapデータの確認と整理
- [ ] P8 GoProデータの確認と整理
- [ ] フェーズ1: Mocap処理 + ブレードアノテーション
- [ ] フェーズ2: ブレード抽出 + cam19リファインメント
- [ ] フェーズ3: GoProパイプライン（同期 + キャリブレーション + GT）
- [ ] 全P8セッションのキャリブレーション + 同期を検証

### データセット動作アノテーション
- [ ] 実験プロトコルから完全な動作リストを確認
- [ ] P1: 全セッションのアノテーション（cam1 GTオーバーレイ、有効区間を記録）
- [ ] P2: 全セッションのアノテーション
- [ ] P3: 全セッションのアノテーション
- [ ] P4: 全セッションのアノテーション
- [ ] P5: 全セッションのアノテーション
- [ ] P6: 全セッションのアノテーション
- [ ] P7: 全セッションのアノテーション
- [ ] P8: 全セッションのアノテーション
- [ ] 最終レビュー: CSVの完全性と整合性を確認
